...
module Wenmar
  class ErrorTest < TestCase
    ...
    def test_field_errors_returns_nil_when_empty
      error = Error.new(code: "not_found", message: "x", field_errors: {}, status: 404)
      assert_nil error.field_errors_by_field
    end

    def test_retryable_rate_limited
      error = Error.new(code: "rate_limited", message: "x", field_errors: {}, status: 429)
      assert error.retryable?
    end

    def test_retryable_5xx
      error = Error.new(code: "internal_error", message: "x", field_errors: {}, status: 500)
      assert error.retryable?
    end

    def test_not_retryable_validation
      error = Error.new(code: "validation_failed", message: "x", field_errors: {}, status: 422)
    ...
    def test_not_retryable_limit_exceeded
      error = Error.new(code: "limit_exceeded", message: "x", field_errors: {}, status: 507)
      refute error.retryable?
    end
